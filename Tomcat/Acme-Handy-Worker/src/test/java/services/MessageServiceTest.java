...
public class MessageServiceTest extends AbstractTest {
	...
	public void testDelete() {
		...
		Assert.isTrue((!this.messageBoxRepository.findInBoxActorId(this.actorService.findOne(6368).getId()).getMessages().contains(saved))
			&& (!this.messageBoxRepository.findOutBoxActorId(this.actorService.findOne(6368).getId()).getMessages().contains(saved)) && (!this.messageBoxRepository.findTrashBoxActorId(this.actorService.findOne(6368).getId()).getMessages().contains(saved))
	...
	public void testBroadcast() {
		...
		final Actor recipient1 = this.actorService.findOne(6618);
		final Actor recipient2 = this.actorService.findOne(6619);
		Actor principal;
		...
		for (final MessageBox mb : recipient1.getMessageBoxes())
			if (mb.getName().equals("In box"))
				Assert.isTrue(mb.getMessages().contains(m));
			else
				Assert.isTrue(!(mb.getMessages().contains(m)));

		for (final MessageBox mb : recipient2.getMessageBoxes())
			if (mb.getName().equals("In box"))
				Assert.isTrue(mb.getMessages().contains(m));
			else
				Assert.isTrue(!(mb.getMessages().contains(m)));
