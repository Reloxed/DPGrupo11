...
public class MessageBoxService {
	...
	public MessageBox save(final MessageBox messageBox) {
		...
		if (messageBox.getIsPredefined()) {
			if (!(messageBox.getId() == 0 && (messageBox.getName().equals("In Box") || messageBox.getName().equals("Out box") || messageBox.getName().equals("Spam box") || messageBox.getName().equals("Trsh box")))) {
			} else {
				Assert.isTrue(this.findOne(messageBox.getId()).getIsPredefined());
				Assert.isTrue(this.findOne(messageBox.getId()).getName() == messageBox.getName());
			}
		} else {
			if (messageBox.getId() == 0)
				for (final MessageBox messageB : principal.getMessageBoxes())
