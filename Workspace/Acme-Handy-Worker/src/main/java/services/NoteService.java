...
public class NoteService {
	 ...
	 * result = new Note();
	 * 
	 * if(principal.equals(customer) || principal.equals(referee) ||
	 * principal.equals(handyWorker)){
	 * 
	 * Assert.isTrue(principal.equals(customer) || principal.equals(referee) ||
	 * principal.equals(handyWorker));
	 * 
	 * 
	 ...
	 * 
	 * 
	 * } return result; } /*if(principal.equals(customer)){
	 * Assert.notNull(customer); Assert.isTrue(principal.equals(customer));
	 * 
	 * result = new Note(); report = result.getReport();
	 * 
	 * Assert.isTrue(!report.getIsFinal());
	 * 
	 * result.setPublishedMoment(new Date(System.currentTimeMillis()-1));
	 * result.setCustomerComment(" "); }else if(principal.equals(referee)){
	 * Assert.notNull(referee); Assert.isTrue(principal.equals(referee));
	 * 
	 * result = new Note(); report = result.getReport();
	 * 
	 * Assert.isTrue(!report.getIsFinal());
	 ...
	 * result.setRefereeComment(" ");
	 * 
	 * }else{ Assert.notNull(handyWorker);
	...
	public Note save(Note note) {
		...
		Assert.notNull(note);
		Assert.isTrue(note.getId() == 0);// notes cannot be updated or deleted
											// once they are saved to the
											// database
	 ...
	 * handyWorker = this.handyWorkerService.findByPrincipal();
	 * 
	 * report = note.getReport(); Assert.notNull(report);
	 * 
	 * if(principal.equals(customer) || principal.equals(referee)||
	 * principal.equals(handyWorker)){
	 * 
	 ...
	 * 
	 * customerComment = note.getCustomerComment(); refereeComment =
	 * note.getRefereeComment(); handyWorkerComment =
	 * note.getHandyWorkerComment();
	 * 
	 ...
	 * }
	 * 
	 * result = this.noteRepository.save(note); Assert.notNull(result);
	 * 
	 * notes = new ArrayList<Note>(); notes = report.getNotes();
	 * if(!notes.contains(result)){ notes.add(result); report.setNotes(notes); }
	 * 
	 * 
	 * return result; }
	 * 
	 * /*if(principal.equals(customer)){ Assert.notNull(customer);
	 * 
	 ...
	 * result = this.noteRepository.save(note); Assert.notNull(result);
	 * 
	 * }else if(principal.equals(referee)){ Assert.notNull(referee);
	 * 
	 ...
	 * handyWorker = this.handyWorkerService.findByPrincipal();
	 * 
	 * Assert.notNull(note); Assert.isTrue(note.getId() == 0);
	 * 
	 * report = note.getReport(); if(principal.equals(customer)||
	 * principal.equals(referee)|| principal.equals(handyWorker)){
	 * 
	 * notes = report.getNotes();
	 * 
	 * if(notes.contains(note)){ notes.remove(note); report.setNotes(notes); }
	 * 
	 * }
	 */
