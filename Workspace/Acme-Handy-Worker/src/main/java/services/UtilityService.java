...
public class UtilityService {
	...
	public String generateTickerNust() {
		String uniqueTicker = null;
		Calendar date;
		...
		while (unique == false) {
			...
			uniqueTicker = day + "-" + this.randomStringW()+"-"+month+"-"+this.randomStringd()+"-"+year;
			for (final FixUpTask fixUpTask : this.fixUpTaskService.findAll())
				if (fixUpTask.getTicker().equals(uniqueTicker))
					continue;
			for (final Curriculum curriculum : this.curriculumService.findAll())
	...
	public String randomStringW() {

		final String possibleChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghyjklmnopqrstuvwxyz";
		final SecureRandom rnd = new SecureRandom();
		final int length = 2;
