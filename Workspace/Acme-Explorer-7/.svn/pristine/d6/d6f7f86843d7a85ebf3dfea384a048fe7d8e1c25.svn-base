...
<a href="message/administrator/create.do"><spring:message code="folder.broadCastMessage" /></a>
<br>
</security:authorize>
...
<br>
<br>
<jstl:if test="${!currentFolder.isSystem }" >
	<input type="button" name="editFolder"
				value="<spring:message code="folder.editFolder" />"
				onclick="redirect: location.href = 'folder/actor/edit.do?folderId=${currentFolder.id}';" />
</jstl:if>
</jstl:if>
