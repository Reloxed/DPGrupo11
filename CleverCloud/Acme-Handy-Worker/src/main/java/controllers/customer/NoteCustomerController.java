...
public class NoteCustomerController extends AbstractController {
	...
	public ModelAndView save(@Valid final Note note, final BindingResult binding) {
		...
		if (binding.hasErrors())
			...
			try {
				final Note noter = this.noteService.save(note);
				result = new ModelAndView("redirect:display.do?noteId=" + noter.getId());
				final Report report = this.reportService.findOne(noter.getReport().getId());
				report.getNotes().add(noter);
			} catch (final Throwable oops) {
				result = this.createEditModelAndView(note, "report.commit.error");
			}
		return result;
	}
