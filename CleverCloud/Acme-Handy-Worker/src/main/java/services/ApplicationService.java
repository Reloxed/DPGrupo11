...
public class ApplicationService {
	...
	public Application save(final Application application) {
		...
		if (actor instanceof HandyWorker) {
			...
			if (application.getId() == 0) { // Not saved in database yet
				...
				Assert.isTrue(registeredMoment.after(fixUpTask.getPublishedMoment())); // The fixUpTask must be
																						// published.
				...
				if (application.getCustomerComment() != null)
					Assert.isTrue(application.getCustomerComment().equals(this.findOne(application.getId()).getCustomerComment()));
			}

			final List<String> atributosAComprobarHW = new ArrayList<>();
			atributosAComprobarHW.add(application.getHandyWorkerComment());

			final boolean containsSpamHW = this.utilityService.isSpam(atributosAComprobarHW);
			...
			Assert.isTrue(application.getRegisteredMoment().equals(this.findOne(application.getId()).getRegisteredMoment()));
			Assert.isTrue(application.getOfferedPrice() == this.findOne(application.getId()).getOfferedPrice());
			Assert.isTrue(application.getHandyWorkerComment() == this.findOne(application.getId()).getHandyWorkerComment());

			if (application.getCustomerComment() != null) {
				final List<String> atributosAComprobarC = new ArrayList<>();
				atributosAComprobarC.add(application.getCustomerComment());

				containsSpamC = this.utilityService.isSpam(atributosAComprobarC);
			}
	...
	public void accept(final Application a, final CreditCard creditCard, final String locale) {
		...
		if (locale.equals("en")) {
			body = "The status of the application of the fix up task whose ticker is " + a.getFixUpTask().getTicker() + " has been changed to " + a.getStatus();
			this.messageService.createAndSaveStatus(a.getApplicant(), body, a.getRegisteredMoment());
		} else {
			body = "El estado de solicitud de la chapuza cuyo identificador es " + a.getFixUpTask().getTicker() + ", se ha cambiado a " + a.getStatus();
			this.messageService.createAndSaveStatus(a.getApplicant(), body, a.getRegisteredMoment());
		}
	...
	public void reject(final Application a, final String locale) {
		...
		if (locale.equals("en")) {
			...
			body = "The status of your application of the fix up task with ticker number " + saved.getFixUpTask().getTicker() + " has been changed to " + saved.getStatus();

			this.messageService.createAndSaveStatus(a.getApplicant(), body, a.getRegisteredMoment());
		} else {

			body = "El estado de solicitud de la chapuza cuyo identificador es  " + saved.getFixUpTask().getTicker() + " ha sido cambiado a " + saved.getStatus();
	...
	public Double findRatioPendingApplications() {
		final Double res = this.applicationRepository.findRatioPendingApplications();
		return res;
	}

	public Double findRatioAcceptedApplications() {
		final Double res = this.applicationRepository.findRatioAcceptedApplications();
		return res;
	}

	public Double findRatioRejectedApplications() {
		final Double res = this.applicationRepository.findRatioRejectedApplications();
		return res;
	}

	public Double findRatioPendingExpiredApplications() {
		final Double res = this.applicationRepository.findRatioPendingExpiredApplications();
		return res;
	}
